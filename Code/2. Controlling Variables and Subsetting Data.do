...
*subset the data to the parts that they care about. This is just a strategy to
...
*use the "drop" command to remove specific variables. In the command below, we
*will keep "unitid", "instnm", "roomamt", "boardamt", "school_type",
*"instruction_expenses", and "research_expenses":
...
*note deleted data, just removed the data from Stata's working memory. The
*original data won't be changed until you save over the file.

*Now, we will remove "school_type", "instruction_expenses", and 
*"research_expenses" variables using a "drop" command:
...
*contains text information. In Excel, any cell can contain number or text, but
...
*Stata makes this extremely easy using the "if" syntax. In the next line, we
*will edit the "roomamt" variable to be blank if it equals the text "NA". This
*will only affect cells that only contain the text "NA":
...
*Note that roomamt should now be a numeric variable. Additionally, we needed
