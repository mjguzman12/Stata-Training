...
collapse (mean) grad_rate_150 ret_pcf instruction_expenses hospital_expenses, by(academic_year)
...
/*MULTIPLE TWOWAY LINES*/
*Now for a really interesting graph! Let's make a twoway graph that plots the
*average university expenditures on instruction AND the average university
*expenditures in each year. To make things a little easier, first divide the
*expenditures by $1 million to make the graph easier to read:
replace instruction_expenses = instruction_expenses/1000000
replace hospital_expenses = hospital_expenses/1000000

*Now, graph it!
twoway (connected instruction_expenses academic_year) || ///
	(connected hospital_expenses academic_year), ///
	legend(label(1 "Instruction Expenditures") label(2 "Hospital Expenditures")) ///
	yscale(r(0 90)) ///
	ylabel(0(10)90) ///
	xlabel(2003(1)2020, angle(45)) ///
	ytitle("Expenditures (Millions USD)") ///
	xtitle("") ///
	title("Average University Expenditures by Category")

*This is a little more complicated than the last twoway example. First, you will notice
*that we have two lines of code to create connected lines in the graph. The first line
*in the code controls the instruction expenditures, and the second line controls the
*hospital expenditures. The syntax here is very finicky. Between the two lines of code
*is a double bar || and after the last line to create the hospital expenditures line
*is a comma. Don't be fooled. Stata is EXTREMELY particular about this syntax and the
*error codes at the time of writing are not intuitive. Following this, we also added
*a line to label the elements of the legend in a more intuitive way. 

*This was a very brief introduction to "twoway" graphs. Twoway graphs are more 
*advanced Stata graphs, and you can do things like add multiple types of graphs
*on top of each other, create multiple axes, and control very minute things about
*the visual in this graphing command.
...
***REGRESSIONS***
*****************
*As we alluded to above, Stata has likely has the best regression syntax in data science.
*Regressions are extremely easy to run in Stata, and many complex models can be run
*right out of the box. To demonstrate this, we will (again) use our cleaned IPEDS data:
use "$intermed/IPEDS Cleaned Data.dta", clear
...
***KEY TAKEAWAYS***
*******************
*1) Stata offers a great deal of syntax to create graphics and Stata defaults aren't preferred
*2) Stata offers the best regression syntax in the business
*3) Stata's programming language Mata is a means to complete more complex tasks
