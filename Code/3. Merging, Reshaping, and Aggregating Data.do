...
*Welcome to Stata Lesson 3! In this lesson, we will primarily review merging, 
...
***AGGREGATING DATA***
**********************
*As usual, let's import some data to get started. Note that because of the
*paths that we set up in the header, the beginning of this command can just
*be written as "$raw"
...
*relies on the (sum), and you could also use (mean), (max) or a different
*statistical operation to aggregate the observations. 
...
*Washington D.C., Marhsall Islands, Puerto Rico, and the US Virgin Islands),
*18 columns (1 column for the state and 1 column for each year between 2003 and
*2019, or 17 columns).

*The Stata syntax to reshape data is fairly straightforward. It is:
...
*One final note is that reshaping is generally not an intuitive concept. This
*takes a lot of practice, and learning whether you need to reshape wide or long
...
***MERGING DATA***
******************
*For the final part of this lesson, we're going to walk through a data merge.
